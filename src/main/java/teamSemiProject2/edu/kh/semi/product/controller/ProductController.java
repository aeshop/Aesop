		...
		try {
			...
			List<Category> category	= service.getCategory();
				...
				if (command.equals("add")) { //post이면서 add: 제품 입력시 data가 text, image로 오게 된다
					...
					//image data는 FileRenamePolicy로 이름이 변경되어 resource 폴더에 저장되고 위치 정보가 product_img 폴더에 저장됨
					...
					String productCategory = mReq.getParameter("productCategory");
					String productDiscount = mReq.getParameter("productDiscount");
					String productStock = mReq.getParameter("productStock");
					String CategoryNum = mReq.getParameter("CategoryNo");
					...
					//먼저 product 시퀀스의 nextVal로 image테이블에서 참조 키로 사용하는 번호를 받아와야한다, 그 후 그 번호를 product와 productImage 객체에 집어넣고 사용한다.
					//위 과정은 서비스 단에서 진행한다.
