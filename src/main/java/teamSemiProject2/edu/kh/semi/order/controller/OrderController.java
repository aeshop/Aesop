		...
		try {
			...
			if(method.equals("GET")) {//get방식 처리
				if(command.equals("view")) {//장바구니 홈페이지로 이동					
					...
					/*
					 * 원래는 로그인한 상태에서 들어가게 되서 로그인이 되어있지 않으면,
					 * 로그인이 필요한 서비스입니다.  메세지가 confirm으로 나오고 로그인 할것인지 아닌지 여부를 알아내야 한다.
					 * - 이거를 어떻게 구현하지? 로그인 체크 자체는 세션으로 가능함, 그러나 바로 url 로 접속하려고 하면?
					 * - 비회원 주문기능이 있어서 그냥 들어가지나, 그렇게 안만들고 싶으면 어떻게 할지 질문해야 할 것으로 보인다.
					 * 
					 * 
					 * 
					 *지금은 테스트용 회원을 만들고 테스트용 회원으로 장바구니에 담고 하는 기능을 만들기
					 * a href :  get으로 접근하더라도 DB에 접근해서 내 회원번호와 맞는 주문 테이블 row를 가지고 와야 한다.
 					 * */
					...
					//사실 회원정보는 세션에 다 있는데 받아올 필요가 있었을까?  - order query를 다시 짜야 한다.(가격, 할인율 가져와야함)  + 지금은 Session안에 정보를 강제로 집어넣기
					req.setAttribute("orderCount", oList.size());
					
				
					//session에 로그인정보 강제 집어넣기					
